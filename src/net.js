...
export const net = {
    ...
    return new Promise((resolve) => {
      ...
      ws.onmessage = (ev) => {
        ...
        if (m.t === "welcome") {
          ...
          done({ solo: false, config: m.config, adminId: m.adminId, maxPlayers: m.maxPlayers, players: m.players || [], pieces: m.pieces || [], id: m.id });
          return;
        }
        if (m.t === "full") { clearTimeout(timeout); done({ full: true, maxPlayers: m.maxPlayers }); return; }
        this._handle(m);
      };
    });
  },
  ...
  _handle(m) {
    ...
    switch (m.t) {
      ...
      case "settings": c.onSettings?.(m.maxPlayers); break;
  ...
  settings(maxPlayers) { this._send({ t: "settings", maxPlayers }); },
